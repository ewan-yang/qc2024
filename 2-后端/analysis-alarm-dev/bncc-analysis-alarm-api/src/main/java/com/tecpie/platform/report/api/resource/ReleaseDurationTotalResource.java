 ...
 * @since 2023-06-28
 */
@Schema(description = "下发时长统计分析 汇总响应结果")
@Getter
@Setter
public class ReleaseDurationTotalResource implements Serializable {
     ...
     * 时长小于2天
     */
    @Schema(description = "时长小于2天")
    private int hourLt2Days;

    /**
     * 时长3至4天
     */
    @Schema(description = "时长3至4天")
    private int hour3Z4Days;

    /**
     * 时长5至6天
     */
    @Schema(description = "时长5至6天")
    private int hour5Z6Days;
     ...
     * 各区域下达时长对比分析图
     */
    @Schema(description = "各区域下达时长对比分析图")
    private Map<String, Map<String, Integer>> regionTimeMap;

    /**
     * 不同用户类型下达时长对比分分析图
     */
    @Schema(description = "不同用户类型下达时长对比分分析图")
    private Map<String, Map<String, Integer>> userTypeTimeMap;

    /**
     * 初始化数据
     */
    public void initData(List<ReleaseDurationStatisticsResource> dataList, Map<String, String> userTypeMap) {
        ...
        // 循环数据
        for (ReleaseDurationStatisticsResource item : dataList) {
            // 所属台区
            ...
            if (hourDays != null) {
                if (hourDays < 2) {
                    this.setHourLt2Days(this.getHourLt2Days() + 1);
                    timeIntervalMap.merge("时长小于2天", 1, Integer::sum);
                    regionMap.merge("时长小于2天", 1, Integer::sum);
                    userTypeVlueMap.merge("时长小于2天", 1, Integer::sum);
                } else if (hourDays <= 4) {
                    this.setHour3Z4Days(this.getHour3Z4Days() + 1);
                    timeIntervalMap.merge("时长3至4天", 1, Integer::sum);
                    regionMap.merge("时长3至4天", 1, Integer::sum);
                    userTypeVlueMap.merge("时长3至4天", 1, Integer::sum);
                } else if (hourDays <= 6) {
                    this.setHour5Z6Days(this.getHour5Z6Days() + 1);
                    timeIntervalMap.merge("时长5至6天", 1, Integer::sum);
                    regionMap.merge("时长5至6天", 1, Integer::sum);
                    userTypeVlueMap.merge("时长5至6天", 1, Integer::sum);
                } else {
