...
//       list: [
//         {
//           name: "HP LaserJet 400 M401 PCL 6",
//           DriverName: "HP LaserJet 400 M401 PCL 6",
//           PortName: "10.130.111.59",
//           Orientation: "1",
//           PaperSize: "9",
//           PaperLength: "2970",
//           PaperWidth: "2100",
...
//         },
//         {
//           name: "NPICDE0DF(HP LaserJet Pro M706n)",
//           DriverName: "HP Color LaserJet A3/11x17 PCL6 Class Dirver",
//           PortName: "WSD-4e404655-3d4c-4ad3-91b9-cd22baba8c1c8a.003f",
//           Orientation: "1",
//           PaperSize: "9",
//           PaperLength: "2970",
//           PaperWidth: "2100",
...
//         Boxdiv.style.top =
//           Math.max(window.pageYOffset + (obody.offsetHeight - iH) / 2, 0) +
//           "px";
...
function getLodop() {
  ...
  try {
    ...
    LODOP.SelectBox.create = function (iW, iH, onlySelect) {
       if (LODOP.SelectBox.PopDiv) LODOP.SelectBox.closeit();
        var obody =
          document.body ||
          document.getElementsByTagName("body")[0] ||
          document.documentElement;
        ...
        titleDiv.innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;打印";
        titleDiv.onmousedown = function (event) {
          LODOP.SelectBox.initializedrag(event);
        };
        titleDiv.onmousemove = function (event) {
          LODOP.SelectBox.drag_drop(event);
        };
        titleDiv.onmouseup = function () {
          LODOP.SelectBox.stopdrag();
        };
        var icoButton = document.createElement("button");
        titleDiv.appendChild(icoButton);
        icoButton.style.cssText =
        ...
        CloseButton.style.cssText =
          ...
          "/images/c_winclose.png) no-repeat scroll 0 0px;margin-right:5px;position:absolute;height:20px;line-height:100px;width:34px;right:3px;border:0;top:4px";
        CloseButton.onclick = function () {
          LODOP.SelectBox.closeit();
          if (onlySelect && LODOP.On_Return) {
            var selfFunc = LODOP.On_Return;
            LODOP.On_Return(0, -1);
            if (!LODOP.On_Return_Remain && selfFunc === LODOP.On_Return)
        ...
        areaDiv.style.width = iW - 2 + "px";
        areaDiv.style.height = iH - 27 + "px";
        var OKButton = LODOP.creatMyButtonElement("button", "确定");
        Boxdiv.appendChild(OKButton);
        OKButton.style.cssText = "position:absolute;width:80px;height:23px;";
        OKButton.style.left = "110px";
        OKButton.style.top = iH - 64 + "px";
        OKButton.onclick = function () {
          LODOP.SelectBox.clickOK(onlySelect);
        };
        var CancelButton = LODOP.creatMyButtonElement("button", "取消");
        Boxdiv.appendChild(CancelButton);
        CancelButton.style.cssText =
          "position:absolute;width:80px;;height:23px;";
        CancelButton.style.left = "240px";
        ...
        areaDiv.appendChild(
          LODOP.creatLabelElement("span", "选打印机：", 200, 46, 67)
        );
        var oSelect = document.createElement("select");
        Boxdiv.appendChild(oSelect);
        LODOP.SelectBox.selPrinter = oSelect;
        oSelect.style.cssText =
          "position:absolute;size:1;width:212px;left:110px;top:67px;";
        LODOP.Create_Printer_List(oSelect);
        areaDiv.appendChild(
          LODOP.creatLabelElement("span", "打印份数：", 200, 46, 121)
        );
        var oCopies = LODOP.creatMyButtonElement("text", "1");
        Boxdiv.appendChild(oCopies);
        LODOP.SelectBox.selCopies = oCopies;
        oCopies.style.cssText =
          "position:absolute;size:1;width:30px;left:110px;top:120px;";
        LODOP.SelectBox.FrantDiv = document.createElement("div");
        obody.appendChild(LODOP.SelectBox.FrantDiv);
        LODOP.SelectBox.FrantDiv.style.cssText =
          "border:0px;left:0px;top:0px;filter: alpha(opacity=20); position: fixed; opacity: 0.2;-moz-opacity: 0.2; _position: absolute;z-index:91009; over-flow: hidden;";
        if (
          LODOP.Browser.IE &&
          (document.compatMode == "BackCompat" ||
          ...
          LODOP.SelectBox.FrantDiv.style.width = obody.scrollWidth + "px";
          LODOP.SelectBox.FrantDiv.style.height = obody.scrollHeight + "px";
        } else {
          LODOP.SelectBox.FrantDiv.style.width = "100%";
          LODOP.SelectBox.FrantDiv.style.height = "100%";
        }
    };
