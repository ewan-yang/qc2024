...
export function transformAuthRouteToVueRoute(item: AuthRoute.Route) {
  ...
  console.log("transformAuthRouteToVueRoute", hasHref(item), item);
  // 外链路由
  if (hasHref(item)) {
 ...
 * @param menuMap
 */
export function transformAuthRouteToSearchMenus(routes: AuthRoute.Route[], menuMap: AuthRoute.Route[] = []) {
  ...
  return routes.reduce((acc, cur) => {
    ...
    if (cur.children && cur.children.length > 0) {
      transformAuthRouteToSearchMenus(cur.children, menuMap);
    }
    return acc;
  }, menuMap);
}
...
/** 老路由转新路由 */
export function transformRoute(oldRoutes = []) {
  if (!oldRoutes || oldRoutes.length === 0) {
    const { toLogin } = useRouterPush(false);
    toLogin('pwd-login', '/home');
    return;
  }
  const newRouter = [];
  oldRoutes.forEach(route => {
    let componentProp: AuthRoute.RouteComponentType = 'basic';
    if (route.type === 2) componentProp = 'multi';
    if (route.type === 4) componentProp = 'blank';
    if (route.type === 3 || route.type === 6 || route.type === 5) componentProp = 'self';
    const menuItem: AuthRoute.Route = {
      name: route.code,
      path: transformRouteNameToRoutePath(route.code),
    ...
    if (route.children.length > 0) {
      menuItem.children = transformRoute(route.children);
    }
    if (route.type === 5) menuItem.meta.singleLayout = 'basic';
    if (route.type === 6) menuItem.meta.singleLayout = 'blank';
    newRouter.push(menuItem);
  });
  return newRouter;
}
