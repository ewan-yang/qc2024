<template>
    <n-card class="mb-4">
        ...
        <div class="flex flex-justify-around mw-1380">
            <n-form ref="searchFormRef" :model="model" inline class="flex flex-wrap">
                ...
                <n-form-item path="taskCode" label="关联停电计划" label-placement="left">
    ...
    <n-card>
        <n-button type="primary" class="mb"
            @click="exportExcal('/analysis-alarm-service/api/v1/taskAdvanceNotice/searchRepeatPowerCutPageList/export', { condition }, '重复停电告警')">导出</n-button>
        <!-- 表格 -->
        <n-spin :show="loading">
    ...
    <!-- 关联通知单抽屉 -->
    <TaskDetailDrawer ref="modalRef" :transformId="transformDataTask.taskId" transformIdType="task">
    </TaskDetailDrawer>
</template>
...
import { repeatedPowerCutWnPageAPI } from '../../../service/api'

const message = useMessage()

// 关联通知单数据定义---抽屉
let transformDataTask = ref({})
...
async function searchDataFn(condition) {
    const dataArr = await repeatedPowerCutWnPageAPI({ ...pageData, condition })
...
const createColumns = () => [
    ...
    {
        title: '本次停电时间',
        key: 'thisPowerOutageTime',
        width: 130,
    },
    {
        title: '上次停电时间',
        key: 'lastPowerCutTime',
        width: 130,
    },
    {
        title: '停电间隔天数（天）',
...
async function pageInit() {
    loading.value = true
    const dataArr = await repeatedPowerCutWnPageAPI(pageData)
