<template>
  <n-form ref="formElRef" :model="formModel" v-bind="getBindValue">
    <n-grid v-bind="getGrid">
      ...
      <!--提交 重置  按钮-->
      <n-gi v-if="getProps.showActionButtonGroup" span="24" :suffix="false">
        <n-space align="center" justify="end" :style="{ 'margin-left': `${getProps.labelWidth}px` }">
          <n-button v-if="getProps.showSubmitButton" v-bind="getProps.submitButtonOptions" @click="handleSubmit">
            {{ getProps.submitButtonText }}
          </n-button>
          <n-button v-if="getProps.showResetButton" v-bind="getProps.resetButtonOptions" @click="resetFields">
