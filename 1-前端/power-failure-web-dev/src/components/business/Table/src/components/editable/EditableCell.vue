    ...
    <div v-show="isEdit" v-click-outside="onClickOutside" class="flex editable-cell-content">
      ...
      <div v-if="!getRowEditable" class="editable-cell-action">
        <n-icon class="mx-2 cursor-pointer">
          <icon-ant-design:check-outlined @click="handleSubmit" />
        </n-icon>
        <n-icon class="mx-2 cursor-pointer">
          <icon-ant-design:close-outlined @click="handleCancel" />
        </n-icon>
      </div>
    </div>
  </div>
...
import { propTypes, isString, isBoolean, isFunction, isNumber, isArray } from '@/utils';
import clickOutside from '@/directives/clickOutside';
import { EventEnum } from '@/components/business/Table/src/componentMap';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getComponentProps = computed(() => {
      ...
      let value = isCheckValue ? (isNumber(val) && isBoolean(val) ? val : Boolean(val)) : val;
    ...
    watchEffect(() => {
      ...
      if (isBoolean(editable) || isBoolean(unref(getRowEditable))) {
        isEdit.value = Boolean(editable) || unref(getRowEditable);
      }
    });
